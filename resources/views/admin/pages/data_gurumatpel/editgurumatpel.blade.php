        ...
        <div class="col-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('admin-updategurumatpel', $gurumatpel->id) }}" method="POST" class="forms-sample">
                        @csrf
                            ...
                            <select name="matpel_id" class="form-control @error('matpel_id') is-invalid @enderror"
                                ...
                                @foreach ($matpels as $matpel)
                                    <option class="text-capitalize" value="{{ $matpel->id }}"
                                        @if ($gurumatpel->matpel_id == $matpel->id) selected @endif>{{ $matpel->nama_matpel }}
                        ...
                        <div class="form-group">
                            ...
                            <select name="kelas_id" class="form-control @error('kelas_id') is-invalid @enderror"
                                ...
                                @foreach ($kelas as $kls)
                                    <option value="{{ $kls->id }}"
                                        @if ($gurumatpel->kelas_id == $kls->id) selected @endif>
                            ...
                            <select name="guru_id"
                                ...
                                @foreach ($gurus as $guru)
                                    <option class="text-capitalize" value="{{ $guru->id }}"
                                        @if ($gurumatpel->guru_id == $guru->id) selected @endif>{{ $guru->nama }}
                                    </option>
                                @endforeach
                            </select>
